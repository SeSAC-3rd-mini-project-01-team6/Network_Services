# U-29 (상) 3. 서비스 관리 > 3.11 tftp 서비스 비활성화
# 점검 내용: tftp, talk 등의 서비스를 사용하지 않거나 취약점이 발표된 서비스의 활 성화 여부 점검
# 조치 방법: 시스템 운영에 불필요한 서비스(tftp, talk, ntalk) 비활성화
...
def check():
    ...
    log(f"[U29] tftp 서비스 상태 점검 결과: {grep if grep else '설정 없음'}")
    ...
    log(f"[U29] 서비스 상태 점검 disable 설정 점검 결과: disable=yes: {', '.join(yes_list) if yes_list else '없음'}, disable=no: {', '.join(no_list) if no_list else '없음'}")
    ...
    for s in no_list: 
        ...
        log(f"[U29] 서비스 상태 점검 결과: {', '.join(exsits) if exsits else '존재 안 함'}")

    if exsits:
        result["상태"] = "취약"
        result["발견 사항"] = origin_files
        result["권고 사항"] = "시스템 운영에 불필요한 서비스(tftp, talk, ntalk) 비활성화"
    else:
        result["상태"] = "양호"
